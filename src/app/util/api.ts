...
export async function getInventoryItem(id: string): Promise<any> {
    const requestOptions: RequestInit = {
        method: "GET",
        redirect: "follow",
        next: {
            tags: ["GET_PURCHASE_ITEMS"],
        },
    };
...
export async function updateInventoryItem(id: string, quantity: number) {
    const requestOptions: RequestInit = {
        method: "PATCH",
        redirect: "follow",
    };
    ...
    const response = await fetch(
        `${baseUrl}/inventory/${id}?quantity=${quantity}`,
        requestOptions
    );
    revalidateTag("GET_PURCHASE_ITEMS");
    return await response.json();
}
